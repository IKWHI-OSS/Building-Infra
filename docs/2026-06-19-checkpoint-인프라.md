...
- 참고(인스턴스 흡수): 데이터 적재 워크플로(`scripts/INGEST-AGENT.md`)는 **이미 같은 자기검증 루프 골격**(Step→verify→DIAGNOSIS→유계재시도→차단기)이다. '모델 비교'(현 인스턴스)에 더해 이 **적재 루프를 같은 엔진의 또 다른 인스턴스(LLM 없는 결정적 인스턴스)로 흡수**하면 도메인 무관 증명이 강해진다. 범용화(설정화)의 자연스러운 두 번째 검증 대상.
